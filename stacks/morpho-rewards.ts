...
export function addMorphoRewards({ stack, api }: StackContext & { api: Api }) {
  ...
  const getMorphoRewards = new Function(stack, 'get-morpho-rewards-function', {
    handler: 'summerfi-api/get-morpho-rewards-function/src/index.handler',
    runtime: 'nodejs20.x',
    environment: {
  ...
  api.addRoutes(stack, {
    'GET /api/morpho-rewards': getMorphoRewards,
  })
}
