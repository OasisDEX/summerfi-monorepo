...
export function addPortfolioConfig({ stack, api }: StackContext & { api: Api }) {
  const { POWERTOOLS_LOG_LEVEL, DEBANK_API_URL, DEBANK_API_KEY, FUNCTIONS_API_URL } = process.env
  ...
  if (!FUNCTIONS_API_URL) {
    throw new Error('FUNCTIONS_API_URL is required to deploy the portfolio functions')
  }
