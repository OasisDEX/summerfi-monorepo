...
3. **Same package** — `packages/<pkg>/graphql.config.yml`: run `pnpm generate-ts-types` (also runs
   ...
   typed-document-node. Note this file is **git-ignored** (root `.gitignore` `/**/types/graphql/*.ts`
   covers all these legacy clients and `morpho-blue-external-api-client`), so a fresh checkout has no
   generated types until you run codegen — build/typecheck of the package (and its consumers) fails
   until `generate-ts-types` has run at least once.
...
5. **On-chain `OperationsRegistry` (if action names/versions changed)** — the registry entries the
   SDK dispatches against are **not** written by the contract deploy. They are generated by
   `sdk/tools/genStrategyDefinitions` (`pnpm strategy:gen`), which walks `ProtocolPluginsRecord`
   (`@summerfi/protocol-plugins`), the `ActionBuildersConfig` (`@summerfi/order-planner-service`) and
   the refinance strategies (`@summerfi/simulator-service`) and emits a Safe multisend / Tenderly
   batch of `addOperation((bytes32[],bool[],string))` calls. The emitted `bytes32` action hashes
   come from each action's `getVersionedName()`/`getActionHash()`, so **any change to an action's
   name or version** (here in core-contracts, or in a protocol plugin) requires re-running
   `strategy:gen` and submitting the new batch on-chain — otherwise refinance/operations revert
   against a stale registry. This tool is a leaf CLI, imported by nothing; its consumer is the
   on-chain registry.

### Deploy a new SDK client version (`@summerfi/sdk-client`)

The SDK backend is deployed by its own SST app (`@summerfi/sdk-infra`, rooted at `sdk/`), separate
from the repo-root `summerfi-stack`, and it gates deploys on the published client version:

1. **Version gate** — `sdk/sst.config.ts` (and `sst.configv3.ts`) read the `sdk-client` package
   version and **refuse to deploy** unless that exact `X.Y.Z` is present in the
   `SDK_DEPLOYED_VERSIONS_MAP` GitHub-environment variable (validated by the zod schema in
   `sdk/sst-environment.ts`, keyed `vX` → `X.Y.Z`). After bumping `sdk/sdk-client/package.json`
   (and its `sdk/sdk-client/bundle/package.json`), add the new version to `SDK_DEPLOYED_VERSIONS_MAP`
   in the GitHub environment or the deploy aborts with
   `Client pkg version ... is not in the list of deployed versions`.
2. **Versioned routing** — one Lambda is deployed per entry; the gateway route major is derived from
   the semver **first character** (`version.charAt(0)` → `/sdk/trpc/v<N>/{proxy+}` and legacy
   `/api/sdk/v<N>/{proxy+}`), so two versions sharing a major collide on the same route.
3. **Handler wiring** — the Lambda is `sdk/sdk-router-function` (wraps `@summerfi/sdk-server`),
   referenced by **handler-path string** in `sdk/create-backend.ts` / `create-backendv3.ts`, not by
   npm import — renaming the file or its `handler`/`options` exports breaks deploy with no type error.
4. **Env** — SDK env vars are a triple source of truth: the zod schema in `sdk/sst-environment.ts`,
   `turbo.json` `globalEnv`, and `ConfigurationProvider` (`sdk/configuration-provider`); the
   "Missing env variable" error names both `sst-environment` and `turbo.json`. VPC (for
   `EARN_PROTOCOL_DB_CONNECTION_STRING`) is attached only on staging/production. See `sdk/CLAUDE.md`.
