  ...
  await Promise.all(
    userChainRewards.map(async ({ address, chainId, rewardsManagerAddresses }) => {
      ...
      try {
        ...
        const chainPositions = allPositions.find(p => p.chainId === chainId)?.positions.positions ?? []
        ...
        rewardsByUser.set(address, {
          ...
          total: existingRewards.unclaimed + normalizedCurrentRewards + existingRewards.claimed + claimedRewards,
        })
      } catch (error) {
        logger.error(`Error fetching rewards for address ${address} on chain ${chainId}:`, error instanceof Error ? error : String(error))
      }
    })
  )
...
async function handleUsersRoute(
  ...
  const usersAddresses = [...new Set(params.addresses.map(address => address.toLowerCase()))] as Address[]
  ...
  const positionsByAddress = new Map<Address, { totalValueLockedUSD: number; rewards: UserRewardsInfo }>()
  ...
  for (const address of usersAddresses) {
    ...
    positionsByAddress.set(address, { totalValueLockedUSD: 0, rewards: { current: 0, claimed: 0, total: 0 } })
  }
  ...
  for (const { chainId, positions } of allPositions) {
    for (const address of usersAddresses) {
      ...
      const rewardsManagerAddresses = Array.from(
        new Set(
          userPositions.map((position) => position.vault.rewardsManager.id as Address)
        )
      )
      ...
      if (rewardsManagerAddresses.length > 0) {
        ...
        logger.info(`Collected rewards manager addresses for address ${address} on chain ID ${chainId}`);
      }
    }
  }
  ...
  const users = Array.from(positionsByAddress.entries()).map(([address, data]) => {
    const rewardsInfo = rewardsByUser.get(address) ?? { current: 0, claimed: 0, total: 0 }
    return {
      address,
