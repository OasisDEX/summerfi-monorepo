...
import {
  ContractsVersionsMap,
  ContractsVersionsSnapshot,
} from './scripts/contract-versions/common/types'
import fs from 'fs'
...
import {
  ...
  SnapshotTrailingTag,
} from './scripts/contract-versions/common/constants'
...
function loadVersionsSnapshot(params: {
  snapshotName: string
  snapshotDir?: string
}): ContractsVersionsSnapshot | undefined {
  try {
    const snapshotName = getSnapshotFilename(params.snapshotName)
...
export async function updateContractVersions(params: {
  snapshotsDir?: string
  snapshotName: string
}): Promise<{
  ...
  if (neededUpdate) {
    const newVersionsSnapshot = await updateVersionsSnapshot({
      currentVersionsSnapshot,
      previousVersionsSnapshot,
    })

    await persistVersionsSnapshot({
      snapshotsDir: params.snapshotsDir ?? SnapshotDirectory,
      snapshotName: params.snapshotName,
      snapshot: newVersionsSnapshot,
    })
  }
