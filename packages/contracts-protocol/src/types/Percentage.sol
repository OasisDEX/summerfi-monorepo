...
using { add as +, subtract as -, multiply as *, divide as /, 
        lessOrEqualThan as <=, lessThan as <,
        greaterOrEqualThan as >=, greaterThan as > } for Percentage global;
...
function multiply(Percentage a, Percentage b) pure returns (Percentage) {
    return Percentage.wrap(Percentage.unwrap(a) * Percentage.unwrap(b) / PERCENTAGE_FACTOR);
}

function divide(Percentage a, Percentage b) pure returns (Percentage) {
    return Percentage.wrap(Percentage.unwrap(a) * PERCENTAGE_FACTOR / Percentage.unwrap(b));
}
