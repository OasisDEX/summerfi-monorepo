...
library PercentageUtils {
    ...
    function applyPercentage(uint256 amount, Percentage percentage) internal pure returns (uint256) {
        return amount * Percentage.unwrap(percentage) / PERCENTAGE_FACTOR;
    }
     ...
     */
    function toPercentage(uint256 numerator, uint256 denominator) internal pure returns (Percentage) {
        return Percentage.wrap((numerator * Percentage.unwrap(PERCENTAGE_100)) / denominator);
    }
}
