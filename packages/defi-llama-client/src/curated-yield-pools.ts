export const curatedYieldPools: { [key: string]: string } = {
  STETH: '747c1d2a-c668-4682-b9f9-296708a3dd90',
  WSTETH: '747c1d2a-c668-4682-b9f9-296708a3dd90', // for wstETH & stETH we use the same pool - Lido
  RETH: 'd4b3c522-6127-4b89-bedf-83641cdcd2eb', // project: 'rocket-pool'
  CBETH: '0f45d730-b279-4629-8e11-ccb5cc3038b4',
  SDAI: 'c8a24fee-ec00-4f38-86c0-9f6daebc4225', // it's `DAI` pool on DefiLlama
  WEETH: '46bd2bdf-6d92-4066-b482-e885ee172264',
  SUSDE: '66985a81-9c51-46ca-9977-42b4fe7bc6df',
  // EZETH: '',
  OSETH: '4d01599c-69ae-41a3-bae1-5fab896f04c8',
  APXETH: 'acee1e4d-a73c-4e20-98f7-e87c13d446e4',
  MEVETH: '5c96d741-3cdc-4f71-9f19-c808a76e401e',
  WOETH: '423681e3-4787-40ce-ae43-e9f67c5269b3',
  BSDETH: 'ca775845-b68a-4084-8d8d-29c31970a643',
  RSETH: '33c732f6-a78d-41da-af5b-ccd9fa5e52d5',
  RSWETH: 'eff9b43c-a80d-4bfc-9f9e-55e02a8ef619',
  SUPEROETHB: 'f388573e-5c0f-4dac-9f70-116a4aabaf17',
}
