import { useEarnProtocolAccount } from '@/hooks/use-earn-wallet-actions'

/**
 * Custom hook to get the user's wallet address.
 *
 * This hook uses the `useUser` and `useAccount` hooks
...
export const useUserWallet = (): {
  ...
  const { account, isLoadingAccount } = useEarnProtocolAccount()

  if (!account?.address) {
    return { userWalletAddress: undefined, isLoadingAccount }
  }

  return { userWalletAddress: account.address, isLoadingAccount }
}
