...
describe('Protocol Manager', () => {
  ...
  let mockPlugins = [createMockPlugin(ProtocolName.Spark), createMockPlugin(ProtocolName.Maker)]

  beforeEach(async () => {
    ctx = await createProtocolManagerContext()
    mockPlugins = [createMockPlugin(ProtocolName.Spark), createMockPlugin(ProtocolName.Maker)]
  ...
  it('should throw an error when getPool is called with an unsupported protocol', async () => {
    ...
    ).rejects.toThrow('No plugin found for protocol: unsupportedProtocol')
  })
  ...
  it('should throw an error when getPool is called for a chain that is not supported by the plugin', async () => {
    ...
    await expect(
      ...
      `Chain ${unsupportedChainId} is not supported by plugin ${ProtocolName.Spark}`,
    )
  })
