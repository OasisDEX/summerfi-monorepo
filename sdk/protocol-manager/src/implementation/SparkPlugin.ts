...
import {
    ...
    SparkPluginBuilder
} from "../interfaces/sparkPluginBuilder";
import { z } from 'zod';
import { ProtocolPlugin, ChainId, ProtocolManagerContext, sparkEmodeCategoryMap, tokenAmountFromBaseUnit, UNCAPPED_SUPPLY, PRESISION_BI, IPositionId } from "../interfaces/protocolPlugin";
...
class SparkPlugin implements ProtocolPlugin<SparkPoolId> {
    ...
    shcema: z.Schema<SparkPoolId> = z.object({
        protocol: z.object({
            name: z.literal(ProtocolName.Spark),
            chainInfo: z.object({
                name: z.string(),
                chainId: z.custom((value) => this.supportedChains.includes(value as ChainId))
            })
        }),
