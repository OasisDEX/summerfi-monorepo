...
type GetPoolIds<ProtocolPlugins extends ProtocolPlugin<any>[]> = {
  [K in keyof ProtocolPlugins]: ProtocolPlugins[K] extends ProtocolPlugin<infer T> ? T : never
}[number]
type UnPackPromise<T> = T extends Promise<infer U> ? U : T
type MatchProtocol<
  ProtocolPlugins extends ProtocolPlugin<any>[],
...
export class ProtocolManager<ProtocolPlugins extends ProtocolPlugin<any>[]> {
  ...
  public async getPool<PoolId extends GetPoolIds<ProtocolPlugins>>(
    ...
    const plugin: ProtocolPlugin<PoolId> | undefined = this.plugins.find(
