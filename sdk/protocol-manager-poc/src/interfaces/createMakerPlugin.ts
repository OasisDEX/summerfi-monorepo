...
import { stringToHex, encodeFunctionData, getContract } from "viem";
...
import { CreateProtocolPlugin, ProtocolManagerContext, ProtocolPlugin, ChainId, MakerContracts, amountFromWei, amountFromRay, amountFromRad, PRESISION_BI, IPositionId } from "./protocolPlugin";
...
export const createMakerPlugin: CreateProtocolPlugin<MakerPoolId> = (ctx: ProtocolManagerContext): ProtocolPlugin<MakerPoolId> => {
    const plugin: ProtocolPlugin<MakerPoolId> = {
        ...
        getPool: async (makerPoolId: unknown): Promise<MakerLendingPool> => {
            ...
            async function getOsmPrice() {
                const res = ctx.provider.call({
                    account: MakerContracts.SPOT,
                    to: spotRes.priceFeedAddress.value,
                ...
                console.log(`OSM peek result: ${res}`)

                return Price.createFrom({ value: '0', baseToken: collateralToken, quoteToken })
            }
            ...
            async function getGemBalance() {
                ...
                const joinGemBalance = await erc20.read.balanceOf([join])

                console.log(`Gem balance result: ${joinGemBalance}`)

                return TokenAmount.createFromBaseUnit({ token: collateralToken, amount: joinGemBalance.toString() })
            }

            const collaterals = {
                [collateralToken.address.value]: {
                    token: collateralToken,
                    price: await getOsmPrice(),
                    ...
                    tokensLocked: await getGemBalance(), // TODO check the gem balance of join adapter
