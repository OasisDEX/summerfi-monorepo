...
export class ArmadaManagerUtils extends ArmadaManagerShared implements IArmadaManagerUtils {
  ...
  /** @see IArmadaManagerUtils.getProtocolAddresses */
  async getProtocolAddresses(
    params: Parameters<IArmadaManagerUtils['getProtocolAddresses']>[0],
  ): ReturnType<IArmadaManagerUtils['getProtocolAddresses']> {
    const admiralsQuartersAddress = this._deploymentProvider.getDeployedContractAddress({
      contractName: 'admiralsQuarters',
    ...
    return {
      admiralsQuarters: admiralsQuartersAddress.value,
    }
  }
}
