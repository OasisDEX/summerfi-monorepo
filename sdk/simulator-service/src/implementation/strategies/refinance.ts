...
export async function refinace(
    ...
    const isCollateralSwapSkipped = args.position.collateralAmount.token.address === args.targetPool.collateralTokens[0].address
    const isDebtSwapSkipped = args.position.debtAmount.token.address !== args.targetPool.debtTokens[0].address
    ...
    const simulation = await simulator
        ...
        .next(async () => ({
            ...
            inputs: {
                depositAmount: ctx.getReference(isCollateralSwapSkipped ? ['PaybackWithdraw', 'withdrawAmount'] : ['CollateralSwap', 'recievedAmount']),
    ...
    return {
        ...
        targetPosition: Object.values(simulation.positions).find(p => p.pool.protocolId.id === args.targetPool.protocolId.id),
