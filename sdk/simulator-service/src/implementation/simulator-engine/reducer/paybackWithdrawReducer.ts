...
export function paybackWithdrawReducer(step: steps.PaybackWithdrawStep, state: SimulationState): SimulationState {
    ...
    return {
        ...state,
        positions: {
            [step.inputs.position.positionId.id]: depositToPosition(step.inputs.position, getReferencedValue(step.inputs.withdrawAmount))
        },
        steps: {
            [step.name]: step
        },
