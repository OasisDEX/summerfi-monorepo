...
describe('Spark Protocol Plugin', () => {
    ...
    it('should throw an error when calling getPool with an unsupported MakerPoolId', async () => {
        ...
        await expect(sparkProtocolPlugin.getPool(invalidSparkPoolIdUnsupportedChain)).rejects.toThrow('Candidate is not correct')
    });
    ...
    it('should throw an error when calling getPool with chain id missing from ctx', async () => {
        ...
        await expect(sparkProtocolPluginWithWrongContext.getPool(sparkPoolIdMock)).rejects.toThrow(`ctx.provider.chain.id undefined`);
    });
    ...
    it('should throw an error when calling getPool with an unsupported chain ID', async () => {
        ...
        await expect(sparkProtocolPluginWithWrongContext.getPool(sparkPoolIdMock)).rejects.toThrow(`Chain ID ${wrongChainId} is not supported`);
    });
