...
import { DOG_ABI, ILK_REGISTRY_ABI, JUG_ABI, SPOT_ABI, VAT_ABI } from '../maker/abis'
import {
...
export class MockContractProvider implements IContractProvider {
  private map: AddressAbiMapsByProtocol

  constructor(addressAbiMapsByProtocol?: AddressAbiMapsByProtocol) {
    this.map = addressAbiMapsByProtocol ?? defaultAddressAbiMapsByProtocol
  }
  ...
  getContractDef<P extends IProtocol['name'], K extends keyof AddressAbiMapsByProtocol[P]>(
    ...
    return this.map[protocol][contractKey]
  }
}

export const defaultAddressAbiMapsByProtocol: AddressAbiMapsByProtocol = {
  [ProtocolName.Spark]: {
    Oracle: {
