...
export const addArk = publicProcedure.input(z.any()).query(async (opts) => {
  const returnedErrors: string[] = []

  if (opts.input == null) {
    throw SDKError.createFrom({
      reason: 'Invalid set add ark request',
  ...
  if (!isArmadaPoolId(opts.input.poolId, returnedErrors)) {
    throw SDKError.createFrom({
      reason: 'Invalid pool id in add ark request',
  ...
  if (!isAddress(opts.input.ark, returnedErrors)) {
    throw SDKError.createFrom({
      reason: 'Invalid ark in add ark request',
      message: returnedErrors.join('\n'),
      type: SDKErrorType.ArmadaError,
    })
  }
