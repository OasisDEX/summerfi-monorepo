...
function createProtocolVariations(entry: [LendingActions, Protocol[]]): (Actions[])[] {
    ...
    const res = protocols.map((protocol) => {
        const protocolActions = protocol === Protocol.Aave 
            ? AaveActions : protocol === Protocol.Spark 
                ? SparkActions : protocol === Protocol.Maker 
                ? MakerActions : MorphoActions

        switch (action) {
            case LendingActions.DepositBorrow:
                const deposit = [
                    CommonActions.Approve,
                    ...([Protocol.Aave, Protocol.Spark].includes(protocol) ? [(protocolActions as any).SetEmode] : []), // Only Aave and Spark have SetEmode
            ...
            case LendingActions.PaybackWithdraw:
                const payback = [
                    CommonActions.Approve,
                    protocolActions.Payback,
                ]
                const withdraw = [
                    protocolActions.Withdraw,
                ]
                return [
                    [...payback, ...withdraw],
                    payback,
