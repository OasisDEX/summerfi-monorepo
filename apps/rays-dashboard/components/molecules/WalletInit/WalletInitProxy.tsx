...
export default async ({ children }: { children: React.ReactNode }) => {
  const [
    ...
    import('@web3-onboard/ledger'),
    import('@web3-onboard/gnosis'),
    import('@web3-onboard/trezor'),
  ])
  const injected = injectedWalletsModule.default({
    custom: [],
    filter: {},
  })

  const walletLink = coinbaseModule.default()

  const walletConnect = walletConnectModule.default({
  ...
  const gnosis = gnosisModule.default()

  const trezorOptions = {
    email: 'support@summer.fi',
    appUrl: 'https://summer.fi',
  }
  const trezor = trezorModule.default(trezorOptions)
