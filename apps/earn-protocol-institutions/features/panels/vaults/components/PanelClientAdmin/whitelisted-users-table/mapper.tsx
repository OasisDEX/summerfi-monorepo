...
export const whitelistedUsersTableMapper = (users: VaultClientAdminUser[]) => {
  return users.map((user) => {
    return {
      content: {
        name: <TableCellText>{user.name}</TableCellText>,
        address: <TableCellText>{formatAddress(user.address)}</TableCellText>,
        access: <TableCellText>{user.access}</TableCellText>,
        'date-added': <TableCellText>{user.dateAddedOrRevoked}</TableCellText>,
        'total-balance': (
