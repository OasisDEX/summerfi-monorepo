declare const styles: {
  readonly "beachClubTvlChallengeRewardCardWrapper": string;
  readonly "boostWrapper": string;
  readonly "colorfulBackground": string;
  readonly "colorfulBorder": string;
  readonly "contentWrapper": string;
  readonly "earnPill": string;
  readonly "expander": string;
  readonly "footer": string;
  readonly "header": string;
  readonly "headerLeftWrapper": string;
  readonly "headerRightWrapper": string;
  readonly "lockIconWrapper": string;
  readonly "shareWrapper": string;
};
export = styles;

