declare const styles: {
  readonly "actionableCritical": string;
  readonly "actionablePrimary": string;
  readonly "cardWrapper": string;
  readonly "centerText": string;
  readonly "expanderContent": string;
  readonly "expanderDescription": string;
  readonly "expanderTitle": string;
  readonly "portfolioYourStakedSumrCardsWrapper": string;
  readonly "stakeButton": string;
  readonly "textSuccess": string;
  readonly "whyCard": string;
  readonly "whyCardButton": string;
  readonly "whyCardInner": string;
  readonly "whyCardsRow": string;
  readonly "whyGradientBox": string;
  readonly "wrapper": string;
};
export = styles;

