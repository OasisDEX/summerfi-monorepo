...
import { AjnaPoolInterestRateResult, AjnaSubgraphClient } from '@summerfi/ajna-subgraph'

const ONE_YEAR_TIMESTAMP = 365 * 24 * 60 * 60

function calculateBorrowForTimestamp(
  morphoSubgraphResponse: AjnaPoolInterestRateResult,
  timestamp: number,
  days: number,
) {
...
export const getAjnaApy = async (params: {
  poolId: `0x${string}`
  ltv: LTV
  timestamp: number
  logger: Logger
  ...
  const [borrowRate1d, borrowRate7d, borrowRate30d, borrowRate90d, borrowRate] = timestamps.map(
    ([timestamp, days]) => {
      return calculateBorrowForTimestamp(ajnaSubgraphResponse, timestamp, days)
    },
  )
  ...
  return {
    ...
    borrowRate90d,
    borrowRate,
    pool: ajnaSubgraphResponse.poolId,
  }
}
